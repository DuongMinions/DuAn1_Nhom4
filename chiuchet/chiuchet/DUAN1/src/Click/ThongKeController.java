...
//        chartPanel.setPreferredSize(new Dimension(jpnItem.getHeight(), 300));
    ...
    public void setDateToChartThang(JPanel jpnItem){
        ...
        JFreeChart chart = ChartFactory.createLineChart("Thống kê doanh thu năm", "Doanh thu tổng", "Giá tiền",categoryDataset,PlotOrientation.VERTICAL , true, true, true);
        ChartPanel chartPanel = new ChartPanel(chart);
        chartPanel.setPreferredSize(new Dimension(jpnItem.getWidth(), 300));
        jpnItem.removeAll();;
        jpnItem.setLayout(new CardLayout());
    ...
    public void setDateToChartNam(JPanel jpnItem){
        ...
        DefaultCategoryDataset categoryDataset = new DefaultCategoryDataset();
        for (vThongKe thongKe : listItem) {
            categoryDataset.addValue(thongKe.getThanhTien(), "Số Lượng", thongKe.getNgayTaoHoaDon());
        }
        JFreeChart chart = ChartFactory.createMultiplePieChart3D("Thống kê doanh thu tháng", categoryDataset, TableOrder.BY_ROW, true, true, true);
        ChartPanel chartPanel = new ChartPanel(chart);
        chartPanel.setPreferredSize(new Dimension(jpnItem.getWidth(), 300));
        jpnItem.removeAll();;
        jpnItem.setLayout(new CardLayout());
    ...
    public void setDateToChartSoLuongBan(JPanel jpnItem){
        ...
        DefaultCategoryDataset categoryDataset = new DefaultCategoryDataset();
        for (vThongKe thongKe : listItem) {
            categoryDataset.addValue(thongKe.getSoLuong(), "Số Lượng", thongKe.getTenSp());
        }
